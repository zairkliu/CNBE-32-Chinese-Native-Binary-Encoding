...
find_v1_data() {
  for d in "${CNBE_V1_DATA_DIR:-}" /scnet_upload_package_DCU/data /scnet_upload_package/data; do
...
run_arm moe128 v1_moe128_dcu2.yaml "$V1_DATA"/*.cnbe
run_arm dense v1_dense_dcu2.yaml "$V1_DATA"/*.cnbe
run_arm unicode v1_unicode_dcu2.yaml "$OUT/data_unicode/unicode.u32"
if [ "${RUN_MATCHED:-0}" = "1" ]; then
  run_arm dense_matched v1_dense_matched_dcu2.yaml "$V1_DATA"/*.cnbe
fi

eval_arm moe128 v1_moe128_dcu2.yaml "$V1_DATA"/*.cnbe
eval_arm dense v1_dense_dcu2.yaml "$V1_DATA"/*.cnbe
eval_arm unicode v1_unicode_dcu2.yaml "$OUT/data_unicode/unicode.u32"
if [ "${RUN_MATCHED:-0}" = "1" ]; then
  eval_arm dense_matched v1_dense_matched_dcu2.yaml "$V1_DATA"/*.cnbe
fi
