...
for enc in ENCODERS:
    ...
    for idx, tc in enumerate(CASES):
        ...
        with torch.no_grad():
            outputs = model.generate(inputs, max_new_tokens=5, do_sample=False, temperature=0)
        ...
        results[enc]["n"] += 1
        results[enc]["details"].append({"case": idx, "answer": tc[3], "got": parsed, "resp": resp[:60]})
        
        elapsed = time.time() - t0
        print(f"  [{elapsed:4.0f}s] {enc:20s} #{idx}: {'OK' if ok else 'FAIL'} (expect={tc[3]}, got={parsed}) [{resp[:40]}]")

total_t = time.time() - t0
print(f"\nDone: {total_t:.1f}s for {sum(r['n'] for r in results.values())} queries")

print(f"\n{'='*50}")
print(f"RESULTS — merged model, {len(CASES)} cases")
print(f"{'='*50}")
