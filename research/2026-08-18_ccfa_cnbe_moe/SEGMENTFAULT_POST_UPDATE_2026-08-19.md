...
这里的候选集含真值及同页真值字符，用于验证“在受限候选排序条件下，结构字段和上下文是否有增益”。虽然按页做了训练/测试切分，但它不是 OCR 引擎实际输出的 top-N 候选，因此不能称为生产 OCR 结果，也不能据此估计端到端部署收益。

### 4.2 DeepSeek V4 API 提示对照

共有 19 个提示；下表只统计各条件能正常解析的样本，解析率为 68.42%--73.68%：
...
raw CNBE 距离并不如 pretrained embedding。构造候选 pilot 中，CNBE 作为特征进入 reranker 后分数更高；这只支持其作为待进一步验证的特征层，不支持端到端排序器或通用能力结论。
...
1. 用 OCR 引擎实际输出的 top-N 候选重做重排，并固定不可见的测试页；
2. 重跑 Unicode baseline，再补非零码条件 struct accuracy；
3. 古籍 OCR 从 37 页扩到 70 页，并保留输入、真值、候选和错误分析；
4. 补 Glyce / SubChar / CNM-BERT 的直接对照；
5. 等 A800×2 跑完，用最终 eval 结果回填论文。
