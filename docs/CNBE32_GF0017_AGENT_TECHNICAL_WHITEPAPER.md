...
After runtime promotion, the next safe engineering step is not another runtime
write. It is a read-only blocker-resolution plan for the 1,393 retained rows.
...
- `reports/8105_runtime_blocker_resolution_plan.json`
- `reports/8105_RUNTIME_BLOCKER_RESOLUTION_PLAN.md`
- `review_packets/8105_full/8105_runtime_blocker_resolution_queue.csv`

The plan confirms the retained rows are a field-completion and policy problem,
not a reason to discard the human-approved structure layer:

| Resolution class | Rows | Meaning |
|---|---:|---|
| `requires_standard_radical_and_stroke_join` | 964 | Approved Agent structure exists, but radical/stroke fields are blank. |
| `requires_index_allocation_and_source_row_insertion_plan` | 276 | The 8105 character is absent from the current runtime model. |
| `requires_position_sensitive_radical_rule` | 124 | `阝` must be resolved as `阜` or `邑` from side evidence. |
| component / shape / alias review classes | 29 | Component-like or ambiguous labels need explicit policy review. |

The important governance rule is that human force approval does not create
missing numeric CNBE32 fields. Force approval can move a row into a remediation
queue, but it cannot invent a radix code, stroke count, preserved index, or
bitfield value. The row becomes writable only after the missing evidence or
policy decision is materialized and tested.

The recommended remediation order is therefore:

1. Join standard radical and stroke evidence for the 964 blank-field rows.
2. Audit the position-sensitive `阝` rule against decomposition or side
   evidence.
3. Review component-like radical labels and ambiguous shape groups.
4. Extend radical aliases only when evidence and tests support the mapping.
5. Design an insertion/index policy for the 276 rows missing from the current
   runtime model.

This blocker-resolution plan writes zero source rows and rebuilds zero
databases. It exists to prevent the project from repeating the earlier failure
mode where an AI-generated structure field looked complete but did not satisfy
the numeric and evidence requirements needed for a reproducible runtime table.

## 8105 Standardized Runtime Repair

After explicit source-write authorization, the project executed a conservative
standardized runtime repair over the 1,393 retained force-approved 8105 rows.
The repair is generated by:
...
The script uses only rows that can preserve the current runtime index/ext bits
and pass a CNBE32 bitfield roundtrip. It takes structure and stroke count from
the human-approved 8105 Agent packet, resolves radicals through the conservative
radical map or the audited left/right `阝` rule, and uses cached ZDIC records
only as cross-reference context when the approved packet lacks a radical.

Runtime repair results:

| Item | Rows |
|---|---:|
| Prior patched 8105 runtime rows | 6,712 |
| Additional standardized repairs applied | 598 |
| Patched 8105 runtime rows after repair | 7,310 |
...
| Character | Outcome |
|---|---|
| 队 | repaired to `阜 / 4 / 左右 / 3` |
| 玕 | repaired to `王 / 7 / 左右 / 3` |
| 㑇 | remains absent from runtime because no current index/ext exists |
| 冁 | remains legacy runtime pending component/radical policy |
| 刁 | remains legacy runtime pending radical alias policy |

This stage updates runtime data and SQLite databases, but it still does not
authorize tag, GitHub release, PyPI publication, 97,686-row validation, or
promotion of cross-reference context to national-standard evidence.
