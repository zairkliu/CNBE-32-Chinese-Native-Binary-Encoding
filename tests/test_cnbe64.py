...
from cnbe64 import GB18030Status, pack, pointer_for_char, unpack, validate


def test_roundtrip_pack_unpack() -> None:
    code = pack(cnbe32=0x01280000, gb18030_pointer=12439, status=GB18030Status.MAPPED, present=True)
    fields = unpack(code)
    assert fields["version"] == 1
    assert fields["gb18030_pointer"] == 12439
    assert fields["gb18030_present"] == 1
    assert fields["gb18030_status"] == GB18030Status.MAPPED
    assert fields["cnbe32"] == 0x01280000
    assert code == 0x10184BC001280000


def test_validate_rejects_missing_pointer_with_present() -> None:
    code = pack(cnbe32=0, present=True, status=GB18030Status.MAPPED)
