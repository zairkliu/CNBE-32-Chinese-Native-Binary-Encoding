...
def test_agent_preencoding_scans_full_repository_table() -> None:
    ...
    assert report["summary"]["row_count"] == 21178
    assert report["summary"]["unique_chars"] == 21178
    assert report["summary"]["unique_codepoints"] == 21178
    assert report["summary"]["duplicate_chars"] == 0
    assert report["summary"]["duplicate_codepoints"] == 0
    assert checkpoint["last_verified_offset"] == 21177
    assert checkpoint["resume_from"] == 21178
...
def test_unicode_first_gate_passes_for_repository_table() -> None:
    ...
    assert report["summary"]["unicode_status_counts"] == {"PASS": 21178}
...
def test_agent_exposes_gf0017_scope_gap_for_non_8105_rows() -> None:
    ...
    assert report["summary"]["agent_status_counts"]["EVIDENCE_GAP"] == 14317
...
def test_cnbe32_bitfield_review_is_separate_from_unicode_identity() -> None:
    ...
    assert report["summary"]["bitfield_status_counts"]["PASS"] == 12836
