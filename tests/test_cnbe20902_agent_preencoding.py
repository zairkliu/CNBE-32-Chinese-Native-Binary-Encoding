...
def test_agent_preencoding_scans_full_repository_table() -> None:
    ...
    assert report["summary"]["row_count"] == 21184
    assert report["summary"]["unique_chars"] == 21184
    assert report["summary"]["unique_codepoints"] == 21184
    assert report["summary"]["duplicate_chars"] == 0
    assert report["summary"]["duplicate_codepoints"] == 0
    assert checkpoint["last_verified_offset"] == 21183
    assert checkpoint["resume_from"] == 21184
...
def test_unicode_first_gate_passes_for_repository_table() -> None:
    ...
    assert report["summary"]["unicode_status_counts"] == {"PASS": 21184}
...
def test_agent_exposes_gf0017_scope_gap_for_non_8105_rows() -> None:
    ...
    assert report["summary"]["issue_counts"]["outside_8105_gf0017_score_scope"] == 13079
    assert report["summary"]["agent_status_counts"]["EVIDENCE_GAP"] == 14323
    assert report["summary"]["agent_status_counts"]["HUMAN_REVIEW_REQUIRED"] == 292
    assert report["summary"]["agent_status_counts"]["SOURCE_GAP"] == 6559


def test_cnbe32_bitfield_review_is_separate_from_unicode_identity() -> None:
    report, _, _ = build_outputs(DEFAULT_CNBE_INPUT, DEFAULT_GF0017_SCORES)

    assert report["summary"]["bitfield_status_counts"]["PASS"] == 12842
