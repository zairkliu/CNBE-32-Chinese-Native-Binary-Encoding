...
static inline uint32_t cnbe_encode(uint8_t radix, uint8_t stroke, uint8_t struct_type, uint16_t ext) {
    return ((uint32_t)radix << 24) | ((uint32_t)stroke << 19) | ((uint32_t)struct_type << 15) | (ext & 0xF);
}
#endif
