...
## 🧪 前沿概念验证 (PoC) / Cutting-Edge Proof of Concept

**CNBE-32 的独特定位：在 CPU 指令集与底层二进制编码这一极端底层的全栈领域，这是目前已知的唯一探索者。**

### 算法与模型层 / Algorithm & Model
将汉字字形、部首融入 AI 模型已是主流方向。**ChineseBERT (ACL 2021)** 将汉字渲染为图像提取字形特征，融合拼音特征，显著超越传统 BERT。**Glyph2Vec / 部首嵌入** 广泛应用于繁简转换、OCR 纠错和古籍数字化。在软件算法层，利用汉字结构增强 AI 已有成熟应用。

### 数据与标准层 / Data & Standards
**Unicode IDS** 用 `⿰` 等符号描述汉字结构，但仅是文本层面的描述。**CHISE (日本)** 是庞大的汉字结构数据库，用于生僻字检索。**仓颉/五笔** 是人类将字形结构映射到输入法的实践。数据层存在结构描述，但其设计目的不涉及计算机底层的逻辑运算。

### 硬件与系统层 / Hardware & System
在 CPU 指令集、操作系统内核、二进制编码层面，将中文设计为原生计算范式，目前几乎没有同类项目。

**为何无人涉足？**
1. **生态壁垒**：x86/ARM + Unicode/UTF-8 已统治全球，重构成本极高
2. **大模型路径**：现有 LLM 通过海量数据已隐式学会汉字规律，无需在硬件层硬编码先验知识
3. **商业逻辑**：现代 CPU 追求通用性，为单一语言设计专用指令集不符商业利益

### 相似的探索 / Similar Efforts
**伏羲-128** 试图用中文汉字作为 CPU 指令助记符，但多停留在概念或教学阶段，缺乏完整的硬件与系统验证闭环。

### PoC 价值 / Value
CNBE-32 的意义不在于立即替代现有标准，而是作为**一个前沿的概念验证**，向学术界和产业界证明：将人类文字的文化属性与计算机二进制底层直接融合，在边缘计算、小模型 AI 和定制芯片（RISC-V/ASIC）领域具有巨大潜力。

> **中文编码研究在应用层已非常深入，但在物理层/指令集层，CNBE-32 的探索是独一无二的。**
...
|-- docs/                     # 系统架构与规范文档
|   |-- specification/       #   核心编码规范 (v1.0, 7份文档)
...
|-- skill/                   # Codex 实验复现技能
|-- src/                     # CNBE 编码工具与SDK
|   |-- cnbe32/             #   Python SDK (core/skill_table/encoders)
|-- include/                 # C 头文件
|   |-- cnbe32.h            #   CNBE-32 API (位域常量/结构体/指令)
|-- data/                    # 编码数据库
|   |-- radix_table.csv     #   部首编码表
|   |-- stroke_db.csv       #   笔画数据库
|   |-- struct_db.csv       #   结构类型枚举
|   |-- ext_db.csv          #   扩展属性数据库
|-- tests/                   # 测试套件
|-- tools/                   # 开发工具
|   |-- generate_mapping.py #   映射表生成器
|   |-- generate_databases.py # 数据库生成工具
|   |-- validate_encoding.py # 编码验证器
|   |-- run_experiment.py   #   实验运行器
|-- hardware/                # 硬件实现
|   |-- verilog/            #   FPGA RTL (v7.2)
|   |-- spike/              #   Spike 自定义指令 (v7.1.1)
|   |-- simulator/          #   RISC-V 指令模拟器
|-- bindings/                # 跨语言绑定
|   |-- rust/               #   Rust/WASM 绑定
...
|-- v9_jepa_tree/            # JEPA预测实验 (v9.0-v9.4)
|-- v10_5_blackhole/         # 黑洞引力场实验 (v10.5)
|-- v10_6_decision_center/   # 社会决策实验 (v10.6)
|-- v10_7_pretrain_base/     # 预训练底座实验 (v10.7)
|-- v10_8_math_reasoning/    # 数学推理实验 (v10.8)
|-- results/                 # 实验白皮书
