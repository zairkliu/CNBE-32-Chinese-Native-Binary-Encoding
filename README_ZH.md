...
CNBE-32 是一项长期研究计划与开源原型。它提出的问题不是替代 Unicode/GB，
而是：汉字结构能否在字符身份之外成为兼容、可计算的表示层，并先后在数据、SDK、
可审计应用、模型、工具链与系统中得到逐级检验。项目采用 32 位字段代数
（部首/笔画/结构/索引/扩展位）作为一种候选表示；它不是已成立的编码标准。
RISC-V、Verilog、Linux、C++/Python/Rust 与 AI 产物是检验同一结构表示能否跨层
流动的独立原型。它们共同构成研究议程，不等于全部议程已经成功。
...
> 在不替代 Unicode/GB 的前提下，是否能构建一份与规范对齐、可审计的汉字结构表示，
> 让计算系统直接消费，并在结构确实相关的任务中产生可复现价值？

CNBE 不以口号回答这个问题，而按以下阶梯推进：

1. **表示层**：Unicode 对齐、字段语义、golden vectors 与版本化结构数据库，先说明
   一个 CNBE 值究竟表示什么。
2. **证据治理层**：国家规范、来源链、人工审核、不确定性标签与 no-write gate，确保
   每个条目可以追溯和复核。
3. **应用层**：OCR 候选、异体字、形近字、罕见字和古籍复核，构造可被证伪的
   结构敏感任务。
4. **模型层**：将 CNBE 作为特征或路由先验，与口径对齐的强基线比较；模型结果不能
   反过来替数据正确性背书。
5. **系统层**：SDK 与硬件原型只检验可移植性，必须建立在表示与任务证据明确之后。

详细的实验协议、验收门槛与禁止外推的结论见
[研究计划与实验阶梯](./docs/RESEARCH_PROGRAM_AND_EXPERIMENT_LADDER.md)和
[证据状态与主张边界](./docs/EVIDENCE_STATUS_2026-08-19.md)。

## 研究设计：对象、方法与可证伪性

项目的思想起点来自“中文原生人工智能治理”讨论中一个更窄、也更可检验的部分：
中文计算若要形成公共价值，其表示的来源、变换过程与能力边界都必须可以审查。
CNBE **不**主张位域能够解决语言治理，也不主张字形结构本身等于语义；它只抽取一个
研究对象：附着在普通 Unicode 字符之上的、可选的结构描述符。

对字符 \(c\)，令 \(u(c)\) 为其 Unicode 码点，令
\(\phi(c) = (r, s, t, i, e)\) 为版本化结构字段，依次对应部首、笔画数、结构类型、
兼容索引和扩展位。当前的 32 位载体为：

```text
E(c) = (r << 24) | (s << 19) | (t << 15) | (i << 4) | e
```

`u(c)` 始终是字符身份与交换键；`E(c)` 是派生且版本化的特征记录，不是另一套字符集、
Unicode 替代品或语言学定理。兼容索引不再可作寻址键，部首字段仍明确受版本约束；
详见[字段语义冻结 v1.1](./docs/FIELD_SEMANTICS_FREEZE_v1.1.md)。

研究以四个可证伪假设组织：

| 假设 | 被检验的主张 | 何种结果构成失败 |
|---|---|---|
| H1：表示完整性 | 固定 schema 能与 Unicode 对齐，并在多实现间复现。 | golden vector 或版本化来源发生分歧。 |
| H2：任务价值 | 在预注册的结构敏感任务中，CNBE 特征优于已声明基线。 | 留出集无增益、存在泄漏，或增益可由非 CNBE 混杂因素解释。 |
| H3：模型价值 | 在同数据、参数、算力和评测口径下，结构先验带来增量价值。 | 匹配的 token-only 或结构感知基线消除该效应。 |
| H4：系统价值 | 同一语义可跨 SDK 与原型边界传递，且成本可测。 | 跨语言不一致，或对所述系统约束没有可测优势。 |

加权形态距离、双曲算子、HDC/VSA 表示和位级路由只是检验 H2--H4 的候选工具，
并不会预设假设为真。它们的正式定义和性质测试见
[CNBE-32 数学结构](./docs/CNBE32_MATHEMATICAL_STRUCTURE.md)。完整协议见
[研究计划与实验阶梯](./docs/RESEARCH_PROGRAM_AND_EXPERIMENT_LADDER.md)；本次重写的理由见
[README 研究化改版说明](./docs/README_RESEARCH_REDESIGN_RATIONALE_2026-08-19.md)。
...
**研究问题。** 一份兼容 Unicode 的结构描述符能否被审计，并在特定任务中展示价值？
CNBE-32 是检验这一问题的当前载体。
...
项目严格区分研究计划与当前证据：已经验证的表示行为、探索性的应用/模型观察、
尚未证实的硬件或部署方向不能相互替代。目标不是宣告中文计算已经被重建，而是形成
一系列小而可复现、可反驳也可引用的结论。
...
**CNBE-32 是一个兼容型中文结构层的研究原型。** 它探索同一份版本化表示能否在
机器码、指令集、操作系统、编译/解码器、编程语言与应用之间复用。各层是验收标准
不同的实验，而不是“完整中文原生编码基础设施已经完成”的宣称。
