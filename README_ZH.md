...
- 8105 缺失字以“待编码”行补录：`276`（`cnbe=NULL`、`needs_encoding=1`）：这是**尚未写入 CNBE 值**的数据库状态，不等同于“全部未审核”；其中 PENC276 T3 的 108 字已完成人工结构/拆解审核，但按证据政策仍不自动生成编码。
...
该人工审核已完成的范围是 108 字；其余 `168` 个待编码行（包括另有 3 个 T3 行 `PENC_078`–`PENC_080`）不在本轮人工审核范围内。它们与历史迁移期的 348 条双源分歧是不同的工作队列，不能由本轮结论一并关闭。
...
8. 在已完成的 108 字 T3 人工审核基线之外，按[受治理编码工作流](./docs/CNBE276_PENDING_ENCODING_WORKFLOW.md)处理其余 168 行待编码 8105 字符；任何 CNBE 候选生成仍须等待独立的部首、笔画裁决和后续书面授权（[SHA-256 锚定清单](./evidence/8105/PENDING_276_ENCODING_INVENTORY.csv)）。
